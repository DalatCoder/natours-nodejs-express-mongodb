...
### 10.4. Creating a more complex `Tour` model
...
## 13. Make a script to automatically import dev data

We will write a script that read the `dev` `json` data file and then import
them into our `mongodb` database.
...
Then, we define `2 script` to run

```json
  "scripts": {
    "import:tours": "node dev-data/data/import-dev-data.js --import",
...
### 14.7. Refactoring our API features

Write a `class` that take 2 `arguments` in the `constructor`

- `query`: the `mongoose` query for the corresponding `model`: `Tour` or `User`
- `queryString`: the `req.query` object
...
And then, to use the `util class`
